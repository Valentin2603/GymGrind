...
public final class GameController {
    ...
    private void startNewRun() {
        ...
        statusMessage = "Осмотритесь в зале. На тренажерах запускаются скилл-чеки.";
        refreshUi();
    ...
    private String buildSkillCheckStartMessage(SkillCheckSession session) {
        if (session.isSequenceMode()) {
            return session.machine().name()
                    + ": набирайте символы по порядку. Полоса снизу тает со временем: верные клавиши заполняют ее, ошибки срезают прогресс.";
        }

        if (session.requiresMultipleHits()) {
            return session.machine().name()
                    + ": попадите в зеленую зону "
                    + session.requiredHits()
                    + " раз подряд. После каждого успеха зона сужается, а сила ускоряет маркер.";
        }

        return session.machine().name() + ": остановите маркер в зеленой зоне, чтобы засчитать подход.";
    }
    ...
    private void cancelSkillCheck() {
        ...
        gameState = GameState.PLAYING;
        nearbyObject = interactionService.findNearbyObject(player, gameMap);
        statusMessage = "Подход отменен. Можно попробовать еще раз.";
        refreshUi();
    }
}
